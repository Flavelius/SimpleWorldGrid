...
namespace SimpleWorldGrid
{
    ...
    public class SimpleWorldGrid<T> where T : class, IGridEntity<T>
    {
        ...
        readonly List<GridEntry> _allEntries = new List<GridEntry>();
        ...
        {
            RESTART:
            while (_allEntries.Count == 0)
            {
                yield return null;
            }
            _updateTimer.Reset();
            _updateTimer.Start();
            for (var i = _allEntries.Count; i-- > 0;)
            {
                if (i%GetFrameSplit() == 0)
                {
                    yield return null;
                }
                var ge = _allEntries[i];
        ...
        int GetFrameSplit()
        {
            if (_allEntries.Count <= 200)
            {
                return _allEntries.Count;
            }
            return _allEntries.Count/10;
        }

        void ProcessEntity(GridEntry ge)
        {
        ...
        void UpdateSubscriptions(GridEntry ge)
        {
